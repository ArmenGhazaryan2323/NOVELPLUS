...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        const priceElement = document.querySelector(".price_ph");
    
        // Add event listener for when the element is clicked
        priceElement.addEventListener("mousedown", function () {
            // Change color on click (when pressed)
            priceElement.style.color = "rgba(209, 129, 16, 0.778)";
        });
    
        // Add event listener for when the click is released
        priceElement.addEventListener("mouseup", function () {
            // Revert color back to the original after releasing the click
            priceElement.style.color = "rgb(209, 129, 16)";
        });
    
        // Add event listener for when the mouse leaves the element while pressing
        priceElement.addEventListener("mouseleave", function () {
            // Ensure the color is reset if the mouse leaves while holding the click
            priceElement.style.color = "rgb(209, 129, 16)";
        });
    });
